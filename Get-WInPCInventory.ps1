...
$HostsWithError    = New-Object System.Collections.ArrayList
$Cred = Get-Credential
...
if ((Test-connection $CurrentHost -count 1 -quiet) -eq "True")
{
    ...
    If ($islocal -ne $true)
    {
        ...
        {
            If (($error[0] -like "*E_ACCESSDENIED*") -and ($AskForCredentialAgain -eq $true)) 
            {
                $Cred = Get-Credential 
                if ((Get-WmiObject -computername $CurrentHost Win32_OperatingSystem -Credential $Cred) -ne $null) 
                    {InventoryPC $CurrentHost $Cred $islocal} 
            ...
            $HostInfo = New-Object PSObject -property @{
                Host   = [System.Net.Dns]::resolve($CurrentHost)
                Ip     = $CurrentHost
            }
            $HostsWithError += $HostInfo
